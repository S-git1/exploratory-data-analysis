...
matrics_long <- pivot_longer(matrics, cols = -1, names_to = "province", values_to = "pass_rate") # cols=-1 for pivot_longer all columns besides column 1. Alternatively, cols=!year can be used in this case.
...
co2 %>% 
  ggplot(aes(x = Year, y = co2, group=Month)) + 
  geom_line()
...
# EXERCISE: Display the distribution of price conditional on cut and carat.  similar to chick diet and weight ex.
# Try facetting by cut and grouping by carat. Try facetting by carat and grouping by cut. 
# Which do you prefer?

diamonds %>% ggplot(aes(x=price, color=cut_interval(carat, n=5))) +
  geom_freqpoly() +
  facet_grid(cut~., scales = 'free_y')

diamonds %>% ggplot(aes(x=price, color=cut)) +
  geom_freqpoly() +
  facet_grid(cut_interval(carat, n=5)~., scales = 'free_y')
...
diamonds %>% ggplot(aes(x=carat, y=price, group=color, color=color)) +
  geom_point() +
  geom_smooth()
...
pp +
  scale_y_continuous(breaks = seq(from = 0, to = 20000, by = 5000),
                     ...
                     limits = c(0,20001)) # limits = c(0,20001) added because last label R20,000 didn't show on plot
...
diamonds %>%
  ggplot(aes(x = cut_interval(price, n = 5))) +
  geom_bar() + 
  labs(title='Diamond count bracketed by price', 
       subtitle = 'Price bracket has been divided into five equal divisions within range of price',
       x = 'Price range [R]', y = 'Count') +
  scale_x_discrete(labels=c('326 ~ 4 030','4 030 ~ 7 720','7 720 ~ 11 400','11 400 ~ 15 100', '15 100 ~ 18 800'))
...
library(stringr)
library(foreach)
labstr <- foreach(i = unique(cut_interval(diamonds$price, n = 5)), .combine = 'c') %do% {
  toString(i)
}

labsplit <- foreach(i = 1:length(labstr), .combine = 'c') %do% {
  a <- strsplit(gsub('\\[|\\]|\\(','',labstr[i]), split=',')[[1]] # get rid of symbols and split strings by comma
  b <- as.numeric(a) # change number string to numeric
  c <- format(b, big.mark = ' ') # format number with thousand separator as whitespace
  d <- paste(str_trim(c)[1], str_trim(c)[2], sep=' ~ ') # get rid of whitespace from start and end of string, then combine with '~' inbetween
}
# --------------------------------------------------------

diamonds %>%
  ggplot(aes(x = cut_interval(price, n = 5))) + 
  labs(y = 'Count', x = 'Price range [R]') +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_x_discrete(labels=labsplit) +
  geom_bar()
...
library(ggrepel) # package used to prevent label from overlapping with point
diamonds %>% sample_n(10) %>% mutate(ValPerCrt = price/carat, BestVal=ifelse(ValPerCrt==max(ValPerCrt), 'Best Value', NA)) %>%
  ggplot(aes(x = carat, y = price)) + 
  geom_text_repel(aes(label = BestVal)) +
  geom_point()
...
s1 + theme(legend.position = "bottom") + guides(colour = guide_legend(ncols = 8)) # guides() does not do anything?
...
s2 <- matrics_long %>% 
  ggplot(aes(x = year, y = pass_rate, colour = province)) +
  geom_line() + theme_bw() + scale_colour_brewer(palette = "Set3") ; s2 # theme bw seems to change background of plot
...
grid.arrange(p1, p2, p3, layout_matrix = matrix(c(1,2,1,3), nrow = 2)) # matrix(c(1,2,1,3)) gives
# 1 1
# 2 3
